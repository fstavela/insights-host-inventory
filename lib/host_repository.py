...
def add_host(
    ...
    if operation_args is None:
        operation_args = {}
    existing_host = existing_host or find_existing_host(identity, input_host.canonical_facts)
    if existing_host:
...
def _find_host_by_elevated_ids(
    ...
    if immutable_facts:
        if from_hosts is not None:
            existing_hosts = list(
                filter(multiple_canonical_facts_host_query_in_memory(identity, immutable_facts), from_hosts)
            )
    ...
    for target_key in elevated_keys:
        ...
        if from_hosts is not None:
            existing_hosts = list(
                filter(multiple_canonical_facts_host_query_in_memory(identity, target_facts), from_hosts)
            )
            existing_host = existing_hosts[0] if existing_hosts else None
        else:
            existing_host = (
...
def find_host_by_multiple_canonical_facts(
    ...
    if from_hosts is not None:
        ...
        host = existing_hosts[0] if existing_hosts else None
    else:
        host = (
